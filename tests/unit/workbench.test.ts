...
describe("workbench read model", () => {
  ...
  it("records visible goal loop controller policy in actual main-agent chat and orchestrator prompt artifacts only while fresh", async () => {
    ...
    try {
      ...
      const actionsAfterPromptRunsFull = (await getWorkbenchSnapshot({ project: project(), path: tempDir }, { topicId: prepared.topic.changeId }))
  ...
  it("carries a second scheduler worker through current-worker gates and hands refreshed ready targets to IntegrationCheck", async () => {
    ...
    try {
      ...
      const firstAudit = await executeWorkbenchAction({ project: project(), path: tempDir }, { ...firstAuditAction, confirm: true });
      ...
      const firstSourceStatus = await execFileAsync("git", ["status", "--short", "--untracked-files=all"], { cwd: tempDir });
      ...
      const outcomeControllerRefresh = await executeWorkbenchAction({ project: project(), path: tempDir }, { ...outcomeControllerRefreshAction, confirm: true });
      ...
      const outcomeGateReadiness = await executeWorkbenchAction({ project: project(), path: tempDir }, { ...outcomeGateReadinessAction, confirm: true });
