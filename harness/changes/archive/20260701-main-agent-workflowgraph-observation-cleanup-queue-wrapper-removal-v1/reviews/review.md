# Review: main-agent-workflowgraph-observation-cleanup-queue-wrapper-removal-v1

Status: approved.

## Findings

- Subagent review before implementation found the plan direction valid but
  recommended this smaller cleanup before building replay. Main risks recorded:
  `WorkflowRun.status === "created"` was misclassified as `queue-running`, and
  `runTaskQueueSequence` remained protected by module-boundary tests despite no
  production Workbench action depending on it.
...
- Passed: `lint-ecl`, `lint-encoding`, `harness-change reindex/status`, and `harness-evolve check`.
- Selected verification scope: targeted WorkflowGraph observation, module
  boundary, and TaskQueue runtime suites plus standard product and Harness gates.
- Full / aggregate suites run or skipped: full `npm run test` not run; touched
  boundary is covered by targeted suites, `test:fast`, `test:workbench`, and
  build/type/lint.
- Rationale for selected scope: change removes a facade and tightens
  WorkflowGraph observation semantics without changing UI, scheduler execution,
...
- delete: removed legacy `runTaskQueueSequence` wrapper/export.
- reuse: strengthened existing WorkflowGraph observation and TaskQueue lifecycle
  owner instead of adding a replay layer.
- yagni: deferred recovery/replay owner until observation semantics and old
  wrapper are clean.
...
- If applicable, before/after line counts: not recorded; only active-change
  pointers changed.
- If applicable, duplicate current-state fields checked: active change pointers
  align with `harness-change status`.
- If applicable, roadmap/current-direction stale language checked: no roadmap
  content changed.
...
- If applicable, module owners checked: WorkflowGraph observation and TaskQueue
  lifecycle ownership.
- If applicable, moved responsibilities: none.
- If applicable, retained facade responsibilities: `workflow-runtime/code-workflow.ts`
  keeps current non-legacy exports only.
- If applicable, forbidden write-back locations: Workbench UI, confirmation
  queue, action handlers, scheduler runtime, terminal, apply/close, automation
  allowlist.
- If applicable, compatibility surface: `runMainAgentTaskQueueLifecycle`.
- If applicable, behavior path tested: targeted Vitest suites.
- If applicable, follow-up split candidates: WorkflowGraph recovery/replay.
- If applicable, boundary tests or lint checks:
  `tests/unit/workbench-module-boundaries.test.ts`.
- If applicable, compatibility result: public facade no longer exports
  `runTaskQueueSequence`; Workbench production actions already use
  `runMainAgentTaskQueueLifecycle`.
- If applicable, tested with: targeted Vitest, typecheck, lint, build,
  `test:fast`, `test:workbench`.
- If not applicable, reason: not applicable.

## Core Mechanism Reuse Coverage

- Core mechanism reuse / architecture growth control coverage applicable: yes.
- If applicable, existing mechanisms reused or strengthened: WorkflowGraph
  observation, WorkflowRun recovery key checks, TaskQueue lifecycle owner.
...
- If applicable, domain-specific logic location: `workflowgraph-observation.ts`.
- If applicable, shared cross-cutting logic location:
  `runMainAgentTaskQueueLifecycle`.
- If applicable, local framework / state machine / projection / validation / gate avoided: recovery/replay owner is deferred.
- If applicable, public API / facade / Workbench compatibility result:
  `workflow-runtime/code-workflow.ts` keeps current non-legacy runtime exports
  and drops the old queue wrapper.
- If applicable, future-cost reduction result: future replay sees one entrypoint
  and corrected state semantics.
- If applicable, tested with: targeted Vitest, typecheck, lint, build,
  `test:fast`, `test:workbench`.
- If not applicable, reason: not applicable.
...
- If applicable, stale active-path / phase grep: active path is current before
  close.
