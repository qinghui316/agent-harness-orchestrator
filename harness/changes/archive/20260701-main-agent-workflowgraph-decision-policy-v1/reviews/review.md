# Review: main-agent-workflowgraph-decision-policy-v1
...
- Selected verification scope: targeted main-agent WorkflowGraph policy/replay/module-boundary suites, then aggregate project checks.
- Targeted tests: `npx vitest run tests/unit/main-agent-workflowgraph-replay.test.ts tests/unit/main-agent-workflowgraph-observation.test.ts tests/unit/main-agent-workflowgraph-decision-policy.test.ts tests/unit/workbench-module-boundaries.test.ts tests/unit/workbench-task-runtime.test.ts` passed.
- Aggregate checks: `npm run typecheck`, `npm run lint`, `npm run test:fast`, and `npm run build` passed.
- Harness checks so far: `lint-encoding` and `harness-evolve check` passed; `lint-ecl` is rerun after active handoff closeout updates.
- Rationale for selected scope: the change is an internal main-agent WorkflowGraph policy/replay architecture slice with no UI or workflow action behavior changes; these suites cover policy mappings, replay behavior, task runtime regressions, and forbidden boundary imports.
...
- delete: none.
- reuse: existing WorkflowGraph replay, observation evidence, queue evidence, role-loop evidence, and canonical managers.
- yagni: avoided: no UI, action execution, scheduler, free LLM decision policy, or new durable evidence family.
- shrink: simpler alternative checked: keeping replay-local `buildNextObservation` was rejected because it leaves strategy hidden in a projection builder.
- net: Lean already.
...
- Retries or environment failures: one Workbench orchestration-map unit test failed once in the first `npm run test:fast` aggregate run, then passed in targeted rerun and the full `npm run test:fast` rerun.
...
- If applicable, documents checked: `AGENTS.md`, `docs/STATUS.md`, active change closeout files.
- If applicable, before/after line counts: not applicable.
- If applicable, duplicate current-state fields checked: not applicable.
- If applicable, roadmap/current-direction stale language checked: no roadmap wording changed; active handoff points at this active change until close.
- If applicable, archive-ledger content promoted / retained / merged / retired / archive-only: not applicable.
- If applicable, over-budget documents and rationale: not applicable.
- If applicable, tested with: `lint-ecl` after active handoff update.
...
- If applicable, module owners checked: `workflowgraph-replay.ts` now delegates next observation policy to `decision-policy.ts`; `workflowgraph-observation.ts` keeps graph observation evidence ownership.
- If applicable, moved responsibilities: replay-local next-observation strategy moved to the policy owner.
- If applicable, retained facade responsibilities: `decideMainAgentWorkflowGraph` remains inside the graph observation owner but is no longer re-exported from the main-agent orchestration barrel.
- If applicable, forbidden write-back locations: policy and replay do not write files, SQLite, action handlers, scheduler state, terminal sessions, apply/close state, or automation allowlists.
- If applicable, compatibility surface: replay summary output still exposes `nextObservation`, derived through the new policy.
- If applicable, behavior path tested: policy mappings, replay behavior, graph observation evidence import, task runtime, and module-boundary tests.
- If applicable, follow-up split candidates: future richer main-agent decision policy and scheduler candidate policy remain separate changes.
- If applicable, boundary tests or lint checks: `tests/unit/workbench-module-boundaries.test.ts`.
- If applicable, compatibility result: existing graph observation evidence behavior retained.
- If applicable, tested with: targeted Vitest suites, `typecheck`, `lint`, `test:fast`, `build`.
- If not applicable, reason: not applicable.
