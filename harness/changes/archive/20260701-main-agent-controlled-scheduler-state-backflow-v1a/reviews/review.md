# Review: main-agent-controlled-scheduler-state-backflow-v1a
...
- Controlled Scheduler replay now surfaces an unsafe gap when expected
  SchedulerRun-scoped evidence is missing but unscoped evidence is present.
- New controlled Scheduler state backflow owner is read-only and in-memory.
- WorkflowGraph replay carries bounded SchedulerRun/runtime state refs without
  changing canonical WorkflowRun / TaskQueue / TaskRun / AgentTask authority.
- Workbench UI, confirmation queue, action registry, revalidation, automation
  allowlist, Scheduler executors, IntegrationCheck, apply/close, remote, merge,
  PR, and Harness evolution paths were not changed.

## Verification

- Selected verification scope: replay/policy/boundary targeted suites plus
  aggregate product build and Harness checks.
...
- Full / aggregate suites run or skipped: no browser screenshot or Workbench
  release suite was needed because this change is architecture/read-model only
  and does not alter UI or action handlers.
- Rationale for selected scope: changed code is limited to main-agent replay,
  controlled Scheduler evidence readers, and module-boundary assertions.
...
- delete: none in this slice; old execution owners remain canonical.
- reuse: reused `controlled-scheduler-step-replay`, Scheduler repository
  readers, SchedulerRuntime repository readers, WorkflowGraph replay, and
  existing decision-policy gap handling.
- yagni: avoided Scheduler execution, new gates, UI, bridge changes, worker
  graph reconciliation, and IntegrationCheck deep backflow.
- shrink: V1a intentionally stops at latest same-Change SchedulerRun/runtime
  state. WorkerLease / worker / IntegrationCheck backflow remains deferred.
- net: Lean for the requested V1a slice.
...
- If applicable, documents checked: `AGENTS.md`, `docs/STATUS.md`, `docs/CURRENT-DEVELOPMENT-PLAN.md`, active change summary/tasks/review.
- If applicable, before/after line counts: not applicable.
- If applicable, duplicate current-state fields checked: not applicable.
- If applicable, roadmap/current-direction stale language checked: active handoff now points to this change; no roadmap expansion beyond V1a was added.
- If applicable, archive-ledger content promoted / retained / merged / retired / archive-only: not applicable.
- If applicable, over-budget documents and rationale: not applicable.
- If applicable, tested with: Harness reindex/status-equivalent handoff checks through `harness-change reindex` and subsequent close/status commands.
...
- Future feature owner module: `src/main-agent-orchestration/controlled-scheduler-state-backflow.ts`.
- If applicable, module owners checked: main-agent replay owns observation; Scheduler/SchedulerRuntime repositories remain read-only sources; Scheduler executors remain in existing owners.
- If applicable, moved responsibilities: none.
- If applicable, retained facade responsibilities: `controlled-scheduler-step-bridge.ts` remains the only controlled Scheduler execution bridge.
- If applicable, forbidden write-back locations: SchedulerRun, SchedulerRuntime, WorkerLease, TaskRun, IntegrationCheck, SQLite, confirmationQueue, Workbench actions, source root.
- If applicable, compatibility surface: replay summary gains bounded `controlledSchedulerStateBackflow` and `schedulerRunIds` refs.
- If applicable, behavior path tested: replay gap regression, state summary, missing runtime gap, module-boundary assertions.
...
- If applicable, compatibility result: existing targeted and fast suites passed.
- If applicable, tested with: targeted Vitest and `npm run test:fast`.
- If not applicable, reason: not applicable.

## Core Mechanism Reuse Coverage

- Core mechanism reuse / architecture growth control coverage applicable: yes.
- If applicable, existing mechanisms reused or strengthened: controlled-step replay, WorkflowGraph replay, Scheduler repository readers, SchedulerRuntime repository readers, decision-policy gap handling.
- If applicable, new cross-cutting mechanism and owner: a narrow read-only state backflow summary under main-agent orchestration.
- If applicable, why existing mechanisms were insufficient: replay had controlled-step evidence but no bounded current SchedulerRun/runtime state observation input.
- If applicable, domain-specific logic location: main-agent orchestration replay/backflow.
- If applicable, shared cross-cutting logic location: no new shared framework.
- If applicable, local framework / state machine / projection / validation / gate avoided: avoided new Scheduler gate, action payload, execution bridge, durable evidence family, UI, and worker graph framework.
- If applicable, public API / facade / Workbench compatibility result: replay summary is backward-compatible with absent Scheduler evidence.
- If applicable, future-cost reduction result: V1b/V1c can extend a bounded read-only summary instead of reading raw Scheduler evidence in policy.
- If applicable, tested with: targeted replay/policy/boundary suites and `test:fast`.
- If not applicable, reason: not applicable.
...
- If applicable, stale active-path / phase grep: active path points to this change before close.
- If applicable, latest archive / active path alignment: will be finalized by close command.
