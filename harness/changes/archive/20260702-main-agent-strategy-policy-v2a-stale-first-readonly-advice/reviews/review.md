# Review: main-agent-strategy-policy-v2a-stale-first-readonly-advice
...
- Rationale for selected scope: the touched boundary is strategy/resume
  consumption, deterministic WorkflowGraph strategy policy, automation/GoalLoop
  regression, and module-boundary enforcement. `test:fast`, `build`, and
  `test:workbench` cover aggregate product and Workbench contracts.
...
- delete: none.
- reuse: existing strategy decision outlet, strategy consumption bridge,
  scoped automation gate/revalidation, and module-boundary tests were reused.
- yagni: avoided: no LLM call path, no strategy JSONL, no action type, no UI,
  no Scheduler/IntegrationCheck executor, and no allowlist expansion.
- shrink: simpler alternative checked: a pure deterministic fix was
  insufficient because V2b needs a bounded advice contract, but V2a keeps
  advice read-only and ignored for execution.
- net: Lean already.
...
- If applicable, roadmap/current-direction stale language checked: active slice
  points to V2a; V2b remains future and advice is explicitly non-executing.
- If applicable, archive-ledger content promoted / retained / merged / retired / archive-only: not applicable.
- If applicable, over-budget documents and rationale: not applicable.
- If applicable, tested with: targeted strategy tests, aggregate verification,
  and Harness checks before close.
...
- If applicable, module owners checked: `src/main-agent-orchestration/strategy-consumption.ts`,
  `src/main-agent-orchestration/strategy-advice.ts`,
  `src/main-agent-orchestration/decision-policy.ts`, automation handler boundary.
- If applicable, moved responsibilities: stale-first precedence remains in
  strategy/resume consumption; advice validation is isolated in the new
  read-only owner.
- If applicable, retained facade responsibilities: deterministic
  `deriveStrategyDecision(...)` remains the only strategy decision outlet.
- If applicable, forbidden write-back locations: no Workbench UI,
  confirmationQueue, automation allowlist, Scheduler/IntegrationCheck executor,
  terminal, apply/close, or action handler imports from the advice owner.
- If applicable, compatibility surface: strategy decision may carry bounded
  advice metadata, but deterministic kind and mode compatibility remain
  unchanged.
...
- If applicable, boundary tests or lint checks:
  `tests/unit/workbench-module-boundaries.test.ts` verifies advice is not a
  controller/import bridge and automation does not consume advice.
- If applicable, compatibility result: no action registry, revalidation,
  confirmationQueue, allowlist, ToolPolicyGate, Scheduler, IntegrationCheck,
  apply/close, remote, PR, merge, or Harness evolution change.
- If applicable, tested with: targeted Vitest suites, `npm run lint`.
- If not applicable, reason: not applicable.

## Core Mechanism Reuse Coverage

- Core mechanism reuse / architecture growth control coverage applicable: yes.
- If applicable, existing mechanisms reused or strengthened: strategy
  consumption fail-closed checks, existing deterministic strategy policy,
  scoped automation runner/revalidation, and module-boundary checks.
- If applicable, new cross-cutting mechanism and owner:
  `src/main-agent-orchestration/strategy-advice.ts`, deliberately read-only and
  not a controller.
- If applicable, why existing mechanisms were insufficient: there was no
  schema-validated place to carry future LLM strategy advice without risking
  execution authority.
- If applicable, domain-specific logic location: stale-first consumption stays
  in `strategy-consumption.ts`; advice validation stays in
  `strategy-advice.ts`; attachment-only integration stays in
  `decision-policy.ts`.
...
- If applicable, local framework / state machine / projection / validation / gate avoided:
  no new durable ledger, strategy JSONL, gate, action type, UI surface, or
  automation controller.
- If applicable, public API / facade / Workbench compatibility result:
  existing callers remain deterministic unless they explicitly pass
  `strategyAdviceInput`; Workbench does not display or execute advice.
- If applicable, future-cost reduction result: V2b can evaluate bounded advice
  consumption without inventing a second strategy/output shape.
- If applicable, tested with: targeted strategy, automation, GoalLoop, module
  boundary, aggregate suites.
- If not applicable, reason: not applicable.
...
- If applicable, handoff files checked: `AGENTS.md`, `docs/STATUS.md`,
  `docs/CURRENT-DEVELOPMENT-PLAN.md`, active change summary.
- If applicable, stale active-path / phase grep: checked through handoff edits
  and Harness pre-close verification.
- If applicable, latest archive / active path alignment: active path points to
  V2a until close; latest archived product change remains previous slice until
  the close command archives this change.
- If applicable, pending evolution state checked: no pending Harness evolution
  was present at change start.
- If not applicable, reason: not applicable.
