# Review: main-agent-workflowgraph-replay-consumption-roadmap-sync-v1

Status: pass; ready to close.

## Findings

- Subagent Plato review before implementation found no direction blocker, but required plan tightening: broader doc sync, a single owner helper, and tests proving replay consumption is side-effect free.
- Implementation follows that shape: production call sites use `recordMainAgentWorkflowGraphObservationAndReplay(...)`; replay summary remains in-memory and non-executing; no UI/action/scheduler path consumes `nextObservation`.
...
- Selected verification scope: replay/observation/policy tests, Workbench task runtime, module boundary tests, workflow/action revalidation, aggregate fast/workbench, build, and Harness checks.
- Full / aggregate suites run or skipped: `test:fast`, `test:workbench`, typecheck, lint, and build passed; broader slow/release suites were not needed because this change does not alter UI, source apply, scheduler execution, or remote paths.
- Rationale for selected scope: the change is a main-agent read-model wiring slice; the selected suites cover replay correctness, no side effects, old-wrapper absence, Workbench task runtime behavior, and action boundary invariants.
...
- delete: none.
- reuse: existing WorkflowGraph observation, replay summary, and decision policy owners are reused.
- yagni: avoided: no UI, execution, scheduler, prompt context, confirmation queue, or replay artifact.
- shrink: simpler alternative checked: direct call-site additions were rejected because they scatter replay consumption logic.
- net: Lean already.
...
- Extra prompts or reviewer instructions: user explicitly requested subagent review before execution; subagent Plato recommended helper encapsulation, doc drift sync, and no-side-effect tests.
...
- If applicable, documents checked: `docs/CURRENT-DEVELOPMENT-PLAN.md`, `docs/STATUS.md`, `AGENTS.md`, active change files.
- If applicable, before/after line counts: not applicable.
- If applicable, duplicate current-state fields checked: not applicable.
- If applicable, roadmap/current-direction stale language checked: `docs/CURRENT-DEVELOPMENT-PLAN.md` now records completed migration slices and the remaining replay consumption -> Policy V2 -> bridge/recovery/scheduler/retirement route.
- If applicable, archive-ledger content promoted / retained / merged / retired / archive-only: not applicable.
- If applicable, over-budget documents and rationale: not applicable.
- If applicable, tested with: pending.
...
- If applicable, module owners checked: `src/main-agent-orchestration/workflowgraph-replay-consumption.ts` owns observation+replay composition; planning and TaskQueue lifecycle no longer scatter replay construction.
- If applicable, moved responsibilities: composition of graph observation + replay summary moves to one main-agent helper.
- If applicable, retained facade responsibilities: `src/main-agent-orchestration/index.ts` remains the public barrel.
- If applicable, forbidden write-back locations: Workbench UI, confirmation queue, action handlers, scheduler runtime, terminal, apply/close modules, automation allowlist, SQLite, and replay artifact files.
- If applicable, compatibility surface: existing observation and replay APIs remain.
- If applicable, behavior path tested: planning observation call-site boundary tests and TaskQueue pause behavior test with replay summary construction.
- If applicable, follow-up split candidates: Policy V2 and scheduler candidate policy remain separate changes.
- If applicable, boundary tests or lint checks: `tests/unit/workbench-module-boundaries.test.ts`.
- If applicable, compatibility result: existing observation and replay APIs remain exported; old `runCodeValidateAuditSequence` / `runTaskQueueSequence` production names remain absent.
- If applicable, tested with: targeted Vitest suite, `test:fast`, `test:workbench`, typecheck, lint, build, and Harness checks listed above.
- If not applicable, reason: not applicable.
...
- If applicable, stale active-path / phase grep: active handoff points to this change before close; close will archive and reset active handoff.
- If applicable, latest archive / active path alignment: `harness-change status` reports STATUS aligned.
- If applicable, pending evolution state checked: `harness-evolve check` reports no pending evolution and threshold not reached.
- If not applicable, reason: not applicable.
