...
Phase 8U defines the first Runtime Continuity Layer contract for code runs only. `WorkerSession`, `RuntimeWorkspace`, `EventSource`, and `AgentEventEnvelope` artifacts are additive evidence beside the Run; they do not change `run.json`, raw Codex logs, Workbench projections, action payloads, or workflow truth. Canonical event scope must come from `WorkerSession`, not from raw adapter events. Direct read/append paths fail closed on cross-Change, cross-Run, cross-role, or misplaced evidence; list/projection-style helpers skip malformed evidence. This layer must not be used as a shortcut to parallel execution, child Changes, sandbox backends, permission engines, or Validation/Audit behavior changes.
