...
- Conversation-first Workbench: project folders contain demand conversations, parent-agent transcript surfaces, inline run graph tabs, Workpad summaries, evidence/detail surfaces, and confirmation queues. The default transcript now uses paged reads, virtual rendering, long-message folding, and `@chenglou/pretext` measurement fallback so long conversations do not require rendering every cell at once. Synthetic 50k-cell pressure acceptance passed without Codex tokens or durable large fixtures, so cursor-aware incremental transcript V2 is deferred until measured backend projection cost justifies it.
