...
- Latest archived product change: `harness/changes/archive/20260620-workbench-test-architecture-apply-integration-slow-suite-split/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260620-workbench-test-architecture-apply-integration-slow-suite-split/summary.md`.
...
This file is the short resume point. There is no active ECL change and no pending Harness evolution. The latest product change completed the Workbench apply/integration slow-suite split: local Result Review / Apply / IntegrationCheck / source-refresh flow tests now live in an explicit slow suite that is part of the `test:workbench` contract.
...
Continue Architecture Growth Control from `docs/CURRENT-DEVELOPMENT-PLAN.md`; next Workbench test architecture stages should use a slightly larger capability-domain work package when boundaries are clear, such as maintenance apply or read-model/projection domain splits. For test-only relocation, prefer targeted verification first and reserve repeated full Workbench runs for shared-runtime changes or final close evidence. Keep `README.md` unrelated and untracked unless the user explicitly asks to include it.
...
- Latest product change: `harness/changes/archive/20260620-workbench-test-architecture-apply-integration-slow-suite-split/summary.md`.
