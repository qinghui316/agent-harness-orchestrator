...
- Latest archived product change: `harness/changes/archive/20260620-controlled-scheduler-workpad-routing-posture/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260620-controlled-scheduler-workpad-routing-posture/summary.md`.
...
This file is the short resume point. No active ECL change is open. The latest product change made the Workpad Goal Loop surface reuse the already-derived sanitized controlled Scheduler routing posture, while preserving the right confirmation card, one human-confirmed controlled advance action, and all scheduler/runtime/ToolPolicy/stale-revalidation/action payload/human-gate boundaries. UI-visible product behavior must be verified with real App DOM or browser checks; projection evidence remains supplemental for derivation and edge cases.
...
Start the next structured product-functional slice from the controlled Scheduler / Goal Loop roadmap. Prefer a larger coherent product step over another tiny architecture-only cleanup, but keep the same hard boundaries: no read-model derivation, scheduler runtime, action payload, stale revalidation, ToolPolicyGate, source/apply/close/remote, or Harness evolution behavior changes unless that phase explicitly owns and verifies them. UI-visible behavior needs real App DOM or browser verification. Keep `README.md` unrelated and untracked unless the user explicitly asks to include it.
