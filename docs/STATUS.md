...
- Latest archived product change: `harness/changes/archive/20260620-workbench-test-architecture-maintenance-slow-suite-split/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260620-workbench-test-architecture-maintenance-slow-suite-split/summary.md`.
...
This file is the short resume point. There is no active ECL change and no pending Harness evolution. The latest product change split the residual Workbench maintenance/self-evolution long-path tests into `tests/slow/workbench-maintenance-flow.test.ts`, reused existing Workbench fixtures, and kept the new suite in the explicit `test:workbench:slow` contract.
...
Continue Architecture Growth Control from `docs/CURRENT-DEVELOPMENT-PLAN.md`. Next Workbench test architecture stages should use a slightly larger capability-domain work package when boundaries are clear, such as read-model/projection domain splits. For test-only relocation, prefer targeted verification first and reserve repeated full Workbench aggregate runs for shared-runtime changes or final close evidence. Keep `README.md` unrelated and untracked unless the user explicitly asks to include it.
...
- Latest product change: `harness/changes/archive/20260620-workbench-test-architecture-maintenance-slow-suite-split/summary.md`.
