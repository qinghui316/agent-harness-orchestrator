...
- Latest archived product change: `harness/changes/archive/20260620-workbench-planning-scheduler-prep-test-domain-split/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260620-workbench-planning-scheduler-prep-test-domain-split/summary.md`.
...
This file is the short resume point. No active ECL change remains. Pending Harness evolution remains none. The latest archived product change split non-executing Workbench planning and scheduler-preparation tests into `tests/unit/workbench-planning-scheduler-prep.test.ts`, left worker start/reconcile/validate/audit/fake Codex execution in residual coverage, reused the shared Workbench fixture owner, and kept the new suite in the explicit `test:workbench` contract while excluding it from `test:fast`. The latest Harness evolution reviewed the five-change Workbench test-architecture split window and recorded `keep` with no new rule/template/lint/product runtime change.
...
Continue Architecture Growth Control from `docs/CURRENT-DEVELOPMENT-PLAN.md`. Remaining Workbench test-architecture candidates include the worker-runtime scheduler path, proposal feedback, conversation lifecycle, and AgentTask residual domains. Keep `README.md` unrelated and untracked unless the user explicitly asks to include it.
...
- Latest product change: `harness/changes/archive/20260620-workbench-planning-scheduler-prep-test-domain-split/summary.md`.
