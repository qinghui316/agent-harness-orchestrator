...
- Latest archived product change: `harness/changes/archive/20260620-controlled-scheduler-confirmation-evidence-surface/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260620-auto-evolve-harness-controlled-scheduler-ui-validation-window/summary.md`.
...
This file is the short resume point. There is no active ECL change and no pending Harness evolution. The latest archived product change closed Controlled Scheduler Confirmation Evidence Surface: the real right confirmation card now shows ready refreshed controlled Scheduler evidence refs using existing read-model evidence, with projection and real React App DOM coverage. The latest Harness evolution promoted the repeated UI-validation lesson into Workbench User-Surface Honesty and the review template: UI-visible Workbench behavior needs real App DOM or browser verification when feasible; projection evidence remains supplemental for derivation and edge cases.
