...
- Latest archived product change: `harness/changes/archive/20260619-maintenance-canonical-patch-target-descriptor-render-helper-reuse/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260619-maintenance-canonical-patch-target-descriptor-render-helper-reuse/summary.md`.
...
This file is the short resume point. There is no active ECL change and no pending Harness evolution. The latest archived product slice is Maintenance Canonical Patch Target Descriptor Render Helper Reuse: `src/agent-task/canonical-patch-target-boundary.ts` now owns display-only canonical patch target descriptor summary formatting, and the canonical update proposal plus canonical patch application manifest renderers reuse it while preserving markdown output, artifact JSON/schema, authority, gates, and source behavior. The previous slice, Maintenance Store-backed Artifact Lookup Helper Reuse, left `src/agent-task/maintenance-artifact-store.ts` owning `findMaintenanceArtifactBy` for the six canonical update / canonical patch chain `read...For...` wrappers. The latest Harness evolution remains Auto-evolve Harness Candidate Window Order: it repaired pending evolution candidate-window selection after Maintenance Canonical Patch Application Authority Helper Reuse closed.
