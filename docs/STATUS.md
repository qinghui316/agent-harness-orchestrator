...
- Latest archived product change: `harness/changes/archive/20260620-workbench-test-architecture-task-runtime-domain-split/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260620-workbench-test-architecture-task-runtime-domain-split/summary.md`.
...
This file is the short resume point. There is no active ECL change and no pending Harness evolution. The latest product change split the TaskRun / TaskQueue / WorkflowRun / typed-workflow runtime guard tests into `tests/unit/workbench-task-runtime.test.ts`, reused the shared Workbench fixture owner, and kept the new suite in the explicit `test:workbench` contract. The latest Harness evolution reviewed the five-change Workbench test-architecture split window and recorded `keep` with no new rule/template/lint/product runtime change.
...
Continue Architecture Growth Control from `docs/CURRENT-DEVELOPMENT-PLAN.md`. Next Workbench test architecture stages should use capability-domain work packages when boundaries are clear; remaining candidates include the broad scheduler/planning flow, Goal Loop, proposal feedback, and AgentTask residual domains. For test-only relocation, prefer targeted verification first and reserve repeated full Workbench aggregate runs for shared-runtime changes, script-contract changes, or final close evidence. Keep `README.md` unrelated and untracked unless the user explicitly asks to include it.
