...
- Latest archived product change: `harness/changes/archive/20260620-workbench-test-architecture-demand-worker-unit-domain-split/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260620-auto-evolve-harness-workbench-test-architecture-granularity-window/summary.md`.
...
This file is the short resume point. There is no active ECL change and no pending Harness evolution. The latest Harness evolution completed with result `keep`: existing Architecture Growth Control/Core Mechanism Reuse, Module Boundary, Documentation Entropy, Experience Lifecycle, workflow-truth, ToolPolicyGate, and human-gate rules remain sufficient; future Workbench test convergence should use a larger capability-domain work package when boundaries are clear.
...
Continue Architecture Growth Control from `docs/CURRENT-DEVELOPMENT-PLAN.md`; next Workbench test architecture stages should use a slightly larger capability-domain work package when boundaries are clear, such as maintenance apply, apply/IntegrationCheck domains, or read-model/projection domain splits. Keep `README.md` unrelated and untracked unless the user explicitly asks to include it.
...
- Latest product change: `harness/changes/archive/20260619-maintenance-canonical-patch-application-target-kind-boundary-reuse/summary.md`.
