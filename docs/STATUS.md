...
- Latest archived product change: `harness/changes/archive/20260620-workbench-test-architecture-read-model-unit-domain-split/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260620-workbench-test-architecture-read-model-unit-domain-split/summary.md`.
- Latest archived Harness evolution: `harness/changes/archive/20260620-auto-evolve-harness-workbench-test-architecture-granularity-window/summary.md`.
- Active product phase: none.
- Active Harness evolution phase: none.
- Active close status: not applicable.

This file is the short resume point. There is no active ECL change and no pending Harness evolution. The latest product change split the residual Workbench read-model/projection unit cluster into `tests/unit/workbench-read-model.test.ts`, reused existing Workbench fixtures, and kept the new suite in the explicit `test:workbench` contract.
...
Continue Architecture Growth Control from `docs/CURRENT-DEVELOPMENT-PLAN.md`. Next Workbench test architecture stages should use capability-domain work packages when boundaries are clear; remaining candidates include TaskRun/TaskQueue runtime/action-validation coverage and other residual Workbench domains. For test-only relocation, prefer targeted verification first and reserve repeated full Workbench aggregate runs for shared-runtime changes or final close evidence. Keep `README.md` unrelated and untracked unless the user explicitly asks to include it.
...
- Latest product change: `harness/changes/archive/20260620-workbench-test-architecture-read-model-unit-domain-split/summary.md`.
