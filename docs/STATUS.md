...
- Latest archived product change: `harness/changes/archive/20260618-phase-12t-product-maintenance-canonical-patch-application-manifest/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260618-phase-12t-product-maintenance-canonical-patch-application-manifest/summary.md`.
...
This file is the short resume point. Phase 12T is archived after adding a deterministic, non-executing canonical patch application manifest that binds a human-gated application record to its patch proposal, validates lineage, and reports whether concrete target descriptors exist for a future writer. The latest Harness evolution remains the Phase 12O through Phase 12S window as `keep / independent_review`, with minimal current-doc drift corrections, no new generic Harness rules, and detailed phase history left archive-only.
...
Resume the next structured product slice from `docs/CURRENT-DEVELOPMENT-PLAN.md`. The likely next product-maintenance step is to evolve patch proposals so they can carry concrete target descriptors for a deterministic writer; do not add automatic rewrite behavior until a later accepted gated path exists. Keep `README.md` unrelated and untracked unless the user explicitly asks to include it.
