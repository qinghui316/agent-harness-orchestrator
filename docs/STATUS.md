...
- Latest archived product change: `harness/changes/archive/20260621-workpad-controlled-scheduler-reconfirmation-surface/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260621-workpad-controlled-scheduler-reconfirmation-surface/summary.md`.
...
The latest product change `workpad-controlled-scheduler-reconfirmation-surface` is archived. It extends the existing controlled Scheduler reconfirmation read-model owner into Workpad read-only surfaces so users can see whether the last stopped step, refreshed next candidate, and current confirmation target align before using the right-side human gate. It adds a real center `工作台` tab for the existing Workpad surface, keeps the default `对话` transcript unchanged, hides Workpad execution while reconfirmation is present, and keeps the right confirmation card as the only executable controlled Scheduler gate. It completed targeted Workbench projection/App DOM verification plus typecheck, lint, build, and `test:fast`.
...
Start the next product-functional controlled Scheduler / Goal Loop slice from the current roadmap rather than architecture-only cleanup. A good next slice is to move toward the smallest controlled Scheduler loop runtime boundary that can observe current evidence, recommend or execute only one legal user-confirmed transition, and stop at human gates, while reusing existing Goal Loop, Scheduler, Workbench read-model, stale revalidation, ToolPolicy, validation/audit, and IntegrationCheck owners. Keep the slice larger than a tiny helper-only change when scope and risk are clear. Keep `README.md` unrelated and untracked unless the user explicitly asks to include it.
...
- Latest product/Harness docs change: `harness/changes/archive/20260621-workpad-controlled-scheduler-reconfirmation-surface/summary.md`.
