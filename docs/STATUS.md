...
- Latest archived product change: `harness/changes/archive/20260620-controlled-scheduler-next-candidate-prompt-evidence/summary.md`.
- Latest archived product/Harness docs change: `harness/changes/archive/20260620-controlled-scheduler-next-candidate-prompt-evidence/summary.md`.
...
This file is the short resume point. There is no active ECL change and no pending Harness evolution. The latest archived product change added compact main-Agent prompt/prepared evidence for the Workbench-visible controlled Scheduler next candidate, while preserving packet parity, explicit non-executing authority, and existing human gates. The previous controlled Scheduler confirmation evidence surface still has real React App DOM coverage for its UI-visible right-card behavior. The latest Harness evolution promoted the repeated UI-validation lesson into Workbench User-Surface Honesty and the review template: UI-visible Workbench behavior needs real App DOM or browser verification when feasible; projection evidence remains supplemental for derivation and edge cases.
...
Continue controlled Scheduler / Goal Loop product-function progress with a meaningful user-visible slice. Preserve one confirmed transition per high-impact action, ToolPolicyGate, stale revalidation, IntegrationCheck/apply boundaries, and human gates. UI-visible behavior needs real App/DOM or browser verification when feasible, not projection-only evidence. Keep `README.md` unrelated and untracked unless the user explicitly asks to include it.
