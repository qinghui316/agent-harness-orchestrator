...
Future dynamic orchestration must preserve that product shape. Phase 7H introduced only the proposal boundary: for complex demands, the main Agent should form the Goal brief and review state, then ask Plan Agent to draft a DecompositionPlan / WorkflowPlan that explains whether the demand should remain one Change, become multiple TaskGraph execution units, become multiple child Changes, or stop for clarification. That plan is a proposal artifact shown through the demand conversation and evidence/details surfaces. Phase 7H confirmation records acceptance of the proposal only. Phase 7I adds a non-executing DecompositionReadinessManifest after confirmation so AHO can say whether a later execution layer may safely consume the proposal, which guardrail blocks it, and what next action is allowed. Phase 7J makes that verdict an execution gate: single-change readiness may authorize `code.run`, while sequential taskgraph readiness must first become a TaskQueueProposal. Phase 7L requires that proposal to become a versioned WorkflowGraphPlan before any confirmed queue start. Phase 7K adds a typed WorkflowRun journal for confirmed sequential TaskQueue execution so progress can be reconstructed and resumed only when the current Harness facts still match.
...
Project Harness maintenance is also a product capability. AHO assigns durable
Maintenance and Evolution tasks to native Agents, while one system Skill teaches
evidence-grounded delta and entropy control. Maintenance edits directly without
a Reviewer; every fifth close runs Evolution instead, with one native scorer
child before direct edits.

The automatic managed-project memory loop is:

```text
Change close
-> closeSequence and durable outbox
-> closes 1-4: one serial Maintenance AgentTask
-> close 5: one proposal-first Evolution AgentTask
-> one native score >= 80 with no hard issue
-> assigned Agent directly reconciles the current project Harness
-> Runtime mechanical verification and AgentTask result
```

The Runtime owns the trigger, fixed window, retry evidence, and result; it does
not choose document paths or apply an Agent-authored patch. Project-memory work
does not enter the demand confirmation queue. AHO's own Runtime source, bundled
system Skills, policies, scripts, and product roadmap are outside this managed-
project loop and continue through ordinary structured Changes.

Product-level maintenance must prevent current memory from becoming append-only
history. Raw closeouts and archives remain durable; current guidance stays
compact and provenance-backed. Agents classify candidate experience as Promote,
Retain, Merge, Retire, or Archive-only before deciding whether a direct delta is
needed.
...
- Durable close evidence plus serial native Maintenance and scored Evolution that directly reconcile current project memory.
