...
Agent roles are provider-neutral catalog content. Every model execution selects
one Provider Registry descriptor, verifies the operation capability profile,
and records an immutable ProviderAttempt containing provider, model, capability,
handoff, workspace, and lineage facts. Codex-native role and Skill loading is a
Codex Adapter responsibility; adding a future provider must not add branches to
role, Workflow, timeline, Agent graph, or Workbench owners.

Main remains one logical user-facing Agent across provider switches. Planning,
Coder/Rework, Auditor, Spec-test, Maintenance, Evolution, and Scorer are leaf
attempts bound to their legal workspace and task packet. A provider switch does
not make a new Main truth or reset Workflow evidence; it stops current model
writers, reconciles durable AHO state, and starts a new provider-fixed attempt.
...
Current provider execution uses one native role profile plus one bounded task
packet and on-demand Skills. File-authoring roles write their assigned
filesystem directly: Planning writes run-scoped proposal files, Coder/Rework
write node worktrees, Maintenance writes canonical project Markdown, and
Evolution writes canonical project Markdown only after its proposal passes one
native ECL scorer. Validator remains deterministic Runtime, not an LLM Agent.
Typed model output is reserved for data roles such as Auditor, Spec-Test
Proposer, and Evolution Scorer.
...
The workflow authoring lesson is more specific than "use workflows." In ODWF,
a skill tells the model how to write a workflow script and how `agent(prompt,
opts)` returns data for the script. In AHO, the equivalent guidance must tell
Plan Agent how to draft a typed WorkflowPlan proposal from the main Agent's
Goal brief, state brief, constraints, and scoped evidence. Plan Agent may
choose sequential, pipeline, ready-set, barrier, or human-gated structure, but
the output remains proposal content until user confirmation and Harness
validation.

Do not put all Harness safety metadata into Agent prompts. Leaf prompts should
contain what the leaf Agent must understand: task, acceptance criteria,
relevant evidence, constraints, and output format. Runtime opts and evidence
binding carry worktree/write mode, role, source scope, policy profile,
recovery inputs, validation/audit requirements, and result references. UI stale
checks, Change matching, confirmation target matching, apply/merge/close gates,
and ToolPolicyGate decisions are Harness boundary checks, not Plan Agent
planning content.

Recovery must be Harness-scoped. Reusing a completed leaf result requires the
leaf task input and execution boundary to match the current graph context.
Change, WorkflowGraphPlan id/hash, node ids, role spec, context packet hashes,
accepted planning artifacts, source revision, worktree base, and policy profile
are recovery and evidence-binding inputs, not a mandate to expose every field
to the Agent. A recovered result is still evidence, not trust; validation,
audit, synthesis, and human gates remain required.
...
| Role | Purpose | Outputs | Boundary |
| --- | --- | --- | --- |
| `memory-maintenance-agent` | Reconcile one assigned terminal Change with the current project Harness | direct edits or no-op, verification, concise result | Cannot choose another Change, trigger Evolution, or update Runtime state |
| `harness-evolution-agent` | Reconcile one fixed five-Change window, including the fifth closeout | proposal, direct edits after scoring, verification | Cannot widen the window or edit before a passing score |
| Native Scorer child | Independently score one Evolution proposal against the fixed window | typed score, dimensions, hard issues, summary | Read-only child; not a catalog role or task owner |

Both model roles use one profile, one Runtime task packet, and
`$aho-harness-engineering` on demand. Runtime resolves project/memory roots,
evidence, fixed windows, task lifecycle, and mechanical completion. The Agent
detects the actual Harness structure, decides Create/Update/Already Good and
Promote/Retain/Merge/Retire/Archive-only, edits directly, and verifies.

The automatic sequence is:

```text
close 1-4 -> one serial Maintenance AgentTask each
close 5 -> one serial Evolution AgentTask only
Evolution proposal -> one native scorer child -> score >= 80 -> direct Agent edits
```

There is no per-close Reviewer, patch package, diff approval truth, Runtime
document apply, DocumentLease, or generic path namespace. Filesystem state is
content truth; changed-file capture is provider evidence only.
...
- one close-assigned Maintenance task for closes 1-4;
- one fixed-window Evolution task instead of Maintenance on close 5;
- one native read-only Scorer child created by the Evolution Agent.
...
AgentTask repository, close dispatch, fixed windows, scoring evidence, results,
and role-scoped projection belong in owned `src/agent-task/*`
modules behind the existing facade. Project Harness content is maintained
automatically by assigned Agents without a human confirmation gate. Product
source apply, remote landing, and other workflow transitions retain their own
existing authority and are not widened by project-memory maintenance.
...
Background project-memory tasks do not interrupt the active demand conversation
or enter its confirmation queue. Assigned Maintenance Agents directly reconcile
the current Harness; on every fifth close, Evolution replaces Maintenance and
edits only after one native scorer child accepts the proposal. Product source
workflow transitions retain their existing apply authority.

Role-scoped context projection is mandatory: ordinary planning/coder/validator/
auditor roles do not receive fixed Evolution windows by default. Runtime gives
the assigned Maintenance or Evolution Agent only its task evidence and roots.
