...
Workbench realtime projection is transport-only. A Provider Adapter normalizes
events with provider-qualified project/conversation/graph/attempt/thread/turn/
item/role/AgentTask lineage. The server first upserts the canonical timeline,
then emits the same canonical identity through request-scoped SSE or the
read-only project background subscription. Snapshot reads only calibrate those
same rows. Provider raw JSONL remains diagnostic evidence; it is never a second
UI transcript or workflow trigger.

One provider turn is keyed by `providerId + attemptId + canonicalTurnId`; one
item adds its `canonicalItemId`. An attempt may contain multiple turns. Tool and
reasoning lifecycle updates replace the same item in place, and completion
becomes a durable boundary row. Main, child, and background Agent surfaces use
provider-qualified session/thread identity, never role-only aggregation.
...
| Shared Conversation | user-facing logical session / coordination fact | Project-scoped conversation with one selected provider, provider bindings, delivery cursors, completed-turn sequence, canonical timeline, and current graph scope; it is not the Change id |
| Canonical Timeline | user-history fact | Persisted provider-qualified user, assistant, reasoning-summary, tool, child, input-request, and terminal items; realtime and refresh update the same identities |
| ParentAgentTranscript | projection | User-readable rendering of canonical timeline items for the selected demand; provider raw history never participates directly |
...
| AgentRelationGraph | projection | Server-owned graph of real model Agent parent/child relationships within the current graph scope; Runtime stages never become nodes |
| ProviderAttempt | immutable execution/recovery record | Provider/model/capability/handoff/worktree snapshot for one model execution; it groups turns but is not a timeline row identity |
...
Workbench action handling remains UI/action orchestration, not workflow truth. The former `chat.ts` compatibility entrypoint is retired: exact ConversationService, MainAgentTurnCoordinator, WorkflowConversationBridge, provider lifecycle, action-handler, and Timeline owners now compose the path. This changes ownership only; action ids, stale-target revalidation, ToolPolicyGate, provider calls, Workflow Runtime authority, and user-visible behavior remain unchanged.
...
| MaintenanceProviderRunEvidence | evidence record | Direct Maintenance result, or Evolution proposal/native score/direct-edit result, with verification refs |
...
Foreground AgentTasks represent bounded role work such as coding, audit, and
rework. Background project-memory AgentTasks are one close-assigned Maintenance
task for closes 1-4 or one fixed-window Evolution task for close 5. Evolution
scoring is a native child thread, not another AgentTask.
...
The orchestrator records foreground role tasks and respects the current local
authorization or human gate before product transitions. Assigned project-memory
Agents update their managed project's Harness automatically; AHO system ECL,
bundled Skills, policies, and product source remain outside that path.
...
### Project Harness Maintenance Evidence

Documentation consistency is maintained by close-assigned native Agents. Their
provider threads, changed-file observations, mechanical verification, and task
results are evidence; the resulting filesystem state is current content truth.

### DemandMemoryCloseout / MaintenanceLedgerEntry / MaintenanceProviderRunEvidence
...
The fixed five-close window is Runtime-owned input for one Evolution AgentTask.
Evolution writes a proposal, spawns one read-only native scorer child, and edits
the current Harness only after a score of at least 80 with no hard issue.

### Evolution Proposal / Native Score / DocBudgetReport

The Evolution proposal is natural-language Agent evidence, not a patch or
Runtime command. The native child score is typed evidence bound to that proposal
and fixed window. A changed proposal invalidates the prior score.

Assigned Agents use documentation entropy as a semantic method rather than a
separate doc-budget task producer. They inspect the actual project Harness and
edit it directly; no
document lease, namespace policy, reviewer, diff manifest, or project-memory
apply transaction exists. Closes 1-4 create one serial Maintenance task each.
Close 5 creates only Evolution, which remains proposal-first and requires one
native ECL score of at least 80 with no hard issue before Harness edits.

Maintenance and accepted Evolution Agents may directly update the actual project
Harness owners they discover. They cannot update AgentTask claims, fixed-window
identity, or results; those remain structured Runtime facts. Product source
changes remain under the normal Workflow Runtime and apply path.
...
Skill enablement does not change `confirmationQueue`, the retired GoalLoop surface, Scheduler,
...
Canonical Timeline is the only user-visible conversation history. Provider
normalized events are persisted before the server emits the same canonical
item identity; realtime patches and snapshots upsert that identity rather than
building separate live and durable transcripts. Native child lifecycle events
produce one provider-qualified, clickable process row. The same
`agentSurfaceId` identifies its server-owned Agent Relation Graph node and its
right-side child transcript; Main prose remains unchanged and is never parsed
to infer Agent state, role, or identity.

`ConversationInteraction` is a server-derived current-scope projection over
canonical provider requests, intake clarification, and current Plan facts. It
is not a database or workflow authority. The browser settles an opaque
interaction id through one route; the server restores exact provider or domain
identity and delegates to the owning adapter, intake, or Main Plan handoff.
Pending interactions render only in the shared Composer slot. Terminal answers,
skips, interruptions, and supersessions become sanitized read-only timeline
history. Secret plaintext is never stored in timeline, snapshot, diagnostics,
logs, or restorable frontend drafts.
...
Workbench may present user-facing scheduler stage labels, but those labels are projections over
existing scoped scheduler actions. They do not become a scheduler loop, slot allocator, source
mutation authority, or execution authorization. Every high-impact scheduler transition re-reads
scoped evidence, rejects stale or mismatched targets, preserves decision and audit identity, and
passes ToolPolicyGate plus the applicable human confirmation before moving one legal step.

Provider-backed Main orchestration owns conversational Goal continuity. Accepted planning crosses
the typed Change acceptance boundary before Workflow Runtime or scheduler owners may create runtime
state. Low-conflict work may be proposed for parallel execution only when scheduler evidence proves
independence; dependent work remains sequential or enters the existing rework and IntegrationFix
paths. Completion is audited against the accepted objective and current artifacts, never inferred
from model confidence.

The former Phase 10 GoalLoop decisions, packets, iterations, controller policy, feedback,
preflight, prompt labels, and `planning.goal-loop.*` actions are retired historical evidence. Their
stable constraints remain current: explanatory or projected evidence is not authorization; exact
action and target identity must be revalidated; stale, malformed, delayed, forged, or cross-Change
evidence fails closed; raw diagnostic detail cannot become workflow truth; and ToolPolicyGate plus
human gates retain authority. Current code must express those constraints through Main
orchestration, Change acceptance, Workflow Runtime, scheduler, Workbench action, Validation/Audit,
and Integration owners without restoring a compatibility path.

Multi-worktree execution remains auxiliary runtime evidence. Worktrees isolate development but do
