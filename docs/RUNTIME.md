...
Phase 10K keeps Goal Loop runtime-neutral. It expands the evidence reader so the next-step packet can name the current legal existing gate after worker, validation, audit, rework, integration, completion, or closeout evidence appears. Runtime behavior remains unchanged: the packet does not call handlers, enqueue turns, create workers, run validation/audit, run IntegrationCheck, apply/close, or mutate source.

Phase 10L keeps the same runtime-neutral boundary and adds read-only freshness checks. Prompt context and Workpad projections must not expose a `GoalLoopNextStepPacket` as current guidance when a non-writing current Goal Loop preview would produce a different recommendation or source evidence set. Stale packets are skipped; the next explicit Goal Loop evaluation can write fresh evidence. No runtime action, scheduler loop, worker start, validation/audit, IntegrationCheck, apply/close, or source mutation is triggered by freshness checks.

Phase 10M remains runtime-neutral and adds only a read-only confirmation-parity check. Prompt context and Workpad projections must not expose a packet recommendation unless the selected demand currently has an enabled matching Harness gate with target-compatible ids. The check reads projection state only; it does not write Goal Loop artifacts, enqueue actions, call action handlers, run ToolPolicyGate early, start workers, run IntegrationCheck, apply/close, or mutate source.

Phase 10N remains runtime-neutral and adds only feedback evidence plus a fresh Goal Loop evaluation. A feedback-scoped evaluation may write `GoalLoopFeedback`, `GoalLoopDecision`, `GoalLoopIteration`, `GoalLoopContinuationBrief`, and `GoalLoopNextStepPacket` artifacts, but it must not execute the recommended action, start workers, allocate leases, create worktrees/runs, run IntegrationCheck, apply/close, or mutate source. Feedback text is user evidence and must not override current repository/Harness facts.

Phase 10O stays runtime-neutral and adds only the Workbench submission bridge for that feedback. The UI may pass `feedback` plus `changeId` and `goalLoopNextStepPacketId` into `planning.goal-loop.feedback.evaluate` when the visible packet already matches the current Harness gate. Runtime behavior is unchanged: no scheduler loop, worker start, validation/audit, IntegrationCheck, apply/close, source mutation, or hidden ToolPolicy authorization is triggered by opening or submitting the feedback editor.

Phase 10P remains runtime-neutral. It only clarifies the feedback action result and verifies that the live snapshot after `planning.goal-loop.feedback.evaluate` refreshes visible Goal Loop guidance. It does not add a controller, enqueue continuation, call scheduler/runtime handlers, execute the recommended action, run validation/audit/IntegrationCheck, apply/close, or mutate source.

Phase 10Q remains runtime-neutral while adding controller policy evidence. A controller verdict may be written and projected only as a non-executing explanation of the safe next posture. It must not enqueue a continuation, dispatch the recommended action, call action handlers, run ToolPolicyGate early, start workers, allocate leases, run validation/audit/IntegrationCheck, apply/close, or mutate source. Any concrete transition still goes through the existing scoped Workbench action and human gate.

Phase 10R remains runtime-neutral and adds only an explicit controller-policy refresh surface. Refreshing policy evidence may write `GoalLoopControllerPolicy`, assistant/thread evidence, Workbench decision records, and ToolEventAudit scope, but it must not execute the recommended action, pre-authorize ToolPolicyGate, start scheduler/runtime work, run validation/audit/IntegrationCheck, apply/close, or mutate source. The concrete current Harness gate remains separate and must be confirmed independently.

Phase 10S remains runtime-neutral and only injects latest valid controller policy evidence into main-Agent prompt context. Invalid, stale, cross-lineage, or execution-started policy evidence is skipped. The context may tell the main Agent to explain, wait, suppress stale guidance, or ask for refreshed evidence, but it must not enqueue actions, call handlers, start workers, run validation/audit/IntegrationCheck, apply/close, mutate source, or authorize ToolPolicyGate early.

Phase 10T keeps the same runtime neutrality and verifies the run artifact boundary. Main-Agent `chat.ask` / `orchestrator.plan` runs can label their prompt stack with `goal-loop-controller-policy` and include packet/policy refs in `context.prepared` event data only when the generated context actually includes that current policy. These labels are replay/audit evidence for prompt construction, not permission to execute the recommended action and not workflow truth.

Phase 10U keeps the same runtime neutrality and adds guided gate handoff refs to prompt evidence. When a fresh controller policy recommends the current concrete Harness gate, `context.md` / `prompt.md` may include the gate action type and target ids, and `context.prepared` may record the guided gate scope. These refs are replay/audit evidence only; the concrete Workbench action still performs its own required-target validation, stale revalidation, ToolPolicyGate check, and human confirmation.

Phase 12H keeps the same runtime neutrality for controlled-loop state evidence. Main-Agent `chat.ask` / `orchestrator.plan` runs may label `goal-loop-controlled-loop-state` in `promptStack` and record compact `goalLoopControlledLoopState` refs in `context.prepared` only when the generated Workpad-visible main-Agent context already includes the validated state. The event data must stay compact and must not include the full scheduler-loop snapshot, action scopes, markdown, or action payload material.

Phase 10V remains runtime-neutral and adds `GoalLoopGateReadinessPreflight` as scoped readiness evidence. A preflight may be written only after latest packet / controller policy / current gate lineage and target ids match, the packet is fresh, and the concrete action's required targets are present. It records `concreteGateInvoked=false`, `toolPolicyAuthorizedConcreteGate=false`, and `executionStarted=false`; it must not call the concrete handler, start workers, run IntegrationCheck, mutate source, or authorize ToolPolicyGate early.

Phase 10W is still runtime-neutral. It lets a concrete Workbench action carry `goalLoopGateReadinessPreflightId` as scoped evidence, but the runtime transition remains the concrete action path. The preflight cannot change the target id, cannot satisfy required target validation by itself, cannot authorize ToolPolicyGate, and cannot execute a Goal Loop action recursively.

Phase 10X keeps the same runtime-neutral boundary and anchors Goal Loop freshness to accepted Change artifacts. `spec.md`, `plan.md`, `tasks.md`, and `ac-map.json` content hashes are source evidence for Goal Loop packets; if those hashes drift or disappear, Workpad guidance, main-Agent prompt context, controller/preflight lineage, and assisted concrete gate confirmation treat the packet as stale. This does not add a runtime action, scheduler execution, source-root hash authority, source mutation, apply, close, merge, or child Change behavior.
