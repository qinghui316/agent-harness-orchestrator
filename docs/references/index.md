...
| --- | --- | --- |
| Harness Workflow Runtime, stable multi-Agent scheduling, ready-set/barrier/pipeline execution, and old runner retirement | `docs/design-docs/harness-workflow-runtime-target.md`, then `docs/design-docs/ref-open-dynamic-workflows.md`, `docs/design-docs/ref-symphony.md`, `docs/design-docs/ref-loop-engineering.md`, and `docs/design-docs/ref-openai-codex.md` | The target doc is AHO authority. ODWF provides workflow runtime mechanics and worktree leaf-isolation semantics, Symphony provides dispatch/reconcile lifecycle, Loop Engineering provides objective/conflict looping, and Codex provides executor/provider-event boundaries. |
...
| Codex exec, JSONL, app-server runtime boundary, native Goal, native subagents, completion audit, and budgeted long-running objective | `docs/design-docs/ref-openai-codex.md` | Codex defines external executor/app-server behavior. Native Goal/subagents are provider capabilities and carriers; they do not replace AHO Harness memory, gates, or workflow truth. |
| Goal-driven loop design, conflict-aware parallelism, worktree/subagent/memory loop support | `docs/design-docs/ref-loop-engineering.md` | Loop Engineering explains a main-agent objective loop over external state. AHO maps that to visible Goal briefs plus Harness evidence, not hidden durable Goal state. |
...
| Deterministic WorkflowPlan shape, pipeline/barrier semantics, worktree leaf isolation, run events, and resume journal | `docs/design-docs/harness-workflow-runtime-target.md` and `docs/design-docs/ref-open-dynamic-workflows.md` | AHO's target doc defines the typed Harness runtime; Open Dynamic Workflows supplies the algorithm reference without becoming an AHO dependency, JS script surface, project memory, or human approval source. |
