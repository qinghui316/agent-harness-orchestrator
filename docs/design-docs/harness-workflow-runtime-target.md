...
for transition selection. `workflow-runtime` now uses that graph to derive the
exact single worker-start target before dispatch. It still does not authorize
Scheduler execution, create runtime records, replace `workflow-runtime`
dispatch revalidation, or provide whole-wave automation.
...
## Mature Runtime Relationship Target

The next architecture goal is relationship compression, not another
adapter-only migration. `workflow-runtime` must converge from a collection of
facades into one runtime contract that all workflow modes share:

```text
Workflow Runtime contract
-> domain workflow instances
-> leaf executors
-> journal / evidence
-> Workbench read-only projection
```

`default-code-change`, TaskRun stages, TaskQueue sequential graphs, Scheduler
ready-set/current-step execution, and future ready-set or pipeline modes should
be workflow instances of that contract. They may carry domain-specific inputs,
but they should not each define their own run context, current-step policy,
event family, recovery behavior, or projection authority.

The mature split is:

- runtime contract: run context, current step, barrier, recovery, event/journal
  policy, and stop points;
- domain workflow instance: maps a confirmed artifact or Scheduler/TaskQueue
  state into the runtime contract;
- leaf executor: executes one scoped unit and returns evidence;
- journal/evidence owner: records canonical runtime events and leaf evidence
  references, with compatibility evidence kept clearly separate;
- projection: reads current gate, transition, and evidence views for display
  or user intent only.

New owners or adapters are acceptable only when they replace an older owner,
compress duplicated policy, isolate a genuinely external/provider model, or
make the next similar workflow cheaper to implement. A wrapper that forwards
between two internal AHO modules without deleting or simplifying an existing
path is not convergence.

Runtime code should stop adding new production writes to
`main-agent-orchestration` loop evidence. Historical main-agent replay,
strategy, and evidence helpers may remain only as compatibility read/projection
support until their consumers are migrated. New runtime event or journal needs
belong in the Workflow Runtime / WorkflowRun event policy instead.

Workbench and Main orchestration may consume current gate, transition, readiness, and
evidence views, but they must not compute runtime scheduling policy. If a UI
button, confirmation card, or controlled continuation needs to
know the next legal step, that answer should come from the runtime/domain owner
or a pure contract used by that owner, not from a feature-local projection
branch.

Tests should follow the same relationship. Use contract and behavior tests for
runtime policy, action scope, stale-target rejection, and projection output.
Keep source-string tests only for narrow sentinels such as forbidden import
directions, retired symbol resurrection, or facade thinness. Tests that lock
implementation snippets, import spelling, or private helper names make
compression harder and should not be the primary proof for mature boundaries.
...
## WorkflowPlan Authoring Contract

Open Dynamic Workflows makes the authoring contract explicit: the model writes
a workflow script, the runtime validates deterministic script constraints,
`agent(prompt, opts)` runs leaves, and the journal can replay completed leaf
calls when the same prompt and opts appear again. AHO should provide the same
kind of clarity to Plan Agent, but the carrier is typed AHO artifacts, not a
JavaScript script.

The AHO authoring path is:

```text
Main Agent Goal brief + state brief + scoped evidence
-> Plan Agent writes WorkflowPlan proposal
-> user confirms the proposal
-> Harness validates, normalizes, and locks WorkflowGraphPlan runtime input
-> Workflow Runtime executes leaf prompt + opts calls
-> leaf evidence, validation, audit, and human gates decide advancement
```

`WorkflowPlan` is the Agent-authored proposal. It may describe sequential,
pipeline, ready-set, barrier, or human-gated workflow structure, but it is not
execution authorization. `WorkflowGraphPlan` is the confirmed runtime input.
The compiler layer validates and locks the execution shape; it must not invent
the business goal, split work that Plan Agent did not propose, or become a
second planner.

Leaf inputs should stay simple and ODWF-like:

- `prompt` contains what the leaf Agent must understand: the scoped task,
  acceptance criteria, relevant evidence, constraints, and output contract.
- `opts` contains Harness execution boundaries: role, source scope,
  worktree/write mode, policy profile, recovery inputs, and required
  validation/audit shape.
- `evidence` records what happened: code run, worktree, artifacts,
  validation, audit, integration, and result references.
- `gate target` records high-impact user intent and stale-target checks:
  Workbench action ids, Change id, source scope/hash, plan/graph hash,
  reservation or worktree ids, confirmation target, apply/merge/close target.

Harness safety metadata must not be dumped into Agent prompts merely because it
exists. The Agent sees the context needed to solve the scoped task. Harness
controls the execution boundary, binds evidence, rejects stale UI intents, and
stops at human gates.

User confirmation is target-scoped. Confirming a plan, worker start, apply,
merge, close, or revision means confirming that exact current target after
server-side revalidation. Workbench buttons and pending composers are user
intent surfaces; the server must re-read current evidence and fail closed when
Change, source scope, plan/graph hash, reservation, worktree, or confirmation
...
`workflow-runtime`. TaskQueue, Scheduler, Plan handoff, and future
...
Skills can teach agents how to read the managed route, project Harness current
Change evidence, canonical product documents, and how to write legal WorkflowPlan drafts. Skills
...
## Replacement Sequence Record

This sequence records the convergence steps evaluated by this design. It is not the current AHO
roadmap; current product priorities live only in `docs/CURRENT-DEVELOPMENT-PLAN.md`.
...
The historical Phase 7 GoalLoop proposal is retired. Its surviving constraint is that the Main
Agent re-reads canonical evidence after each Workflow round, decides whether the Goal is complete,
and if needed asks the Plan Agent for a revised WorkflowPlan; it does not create a separate runtime
state machine.
