...
export async function writeCleanupInventory(repositoryRoot, outputPath = "design-assets/agent-office/cleanup-inventory-before.json") {
  const root = resolve(repositoryRoot);
  const corpus = (await Promise.all(referenceRoots.map((path) => readTextTree(resolve(root, path))))).join("\n");
  const files = [];
  for (const candidate of candidates) {
    for (const path of await listFiles(resolve(root, candidate.root))) {
