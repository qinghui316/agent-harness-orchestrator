...
export async function buildApprovedPropsProof(repositoryRoot) {
  ...
  const approvedRoot = resolve(repositoryRoot, manifest.approvedFrameRoot);
  const outputPath = resolve(repositoryRoot, "design-assets/agent-office/proof/props/approved-v1/approved-props-contact-sheet.png");
  ...
  await sharp({ create: { width: columns * cell.width, height: rows * cell.height, channels: 4, background: { r: 238, g: 240, b: 239, alpha: 1 } } })
    ...
    .png()
    .toFile(outputPath);
  return { outputPath, propCount: manifest.props.length };
}
