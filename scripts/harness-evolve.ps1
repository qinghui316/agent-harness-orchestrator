...
function Get-ArchiveChanges {
  ...
  return @(Get-ChildItem -LiteralPath $archiveDir -Directory -Force | Where-Object { $_.Name -notmatch '^auto-evolve-harness-' } | Sort-Object Name)
}
...
switch ($Command) {
  ...
  "check" {
    ...
    if ($delta -ge [int]$state.archive_threshold) {
      $candidateLines = $archives | Select-Object -Last $delta | ForEach-Object { "- harness/changes/archive/$($_.Name)/summary.md" }
      $lines = @(
