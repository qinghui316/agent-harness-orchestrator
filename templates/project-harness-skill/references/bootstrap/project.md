...
user confirms one language/application variant and the plan is approved, the existing AHO Worker
workflow creates exactly that variant in its assigned worktree. Review all output against the
accepted spec and environment contract. Business source, tests, commands, documentation, and CI
are Worker Change outputs, never project Harness initialization output.
