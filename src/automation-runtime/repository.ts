...
function renderAuthorizationMarkdown(authorization: AutomationAuthorization): string {
  return [
    ...
    `- Current gate: ${authorization.requestedGate.automationCurrentGateActionType ?? authorization.requestedGate.actionType ?? "unknown"}`,
    "",
    "## Boundaries",
    "",
...
function renderIterationMarkdown(iteration: AutomationIteration): string {
  return [
    ...
    `- Submitted action: ${iteration.submittedActionType ?? "none"}`,
