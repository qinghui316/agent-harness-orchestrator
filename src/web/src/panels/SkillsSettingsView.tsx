...
export function SkillsSettingsView({ projectId, productMode, conversationId, providerId, onRefresh }: {
  ...
  useEffect(() => {
    ...
    load().catch((cause: unknown) => {
      if (requestGeneration === requestGenerationRef.current && requestIdentityKey === identityKeyRef.current) setMessage(cause instanceof Error ? cause.message : String(cause));
    });
  ...
  async function run(action: () => Promise<void>): Promise<boolean> {
    ...
    try {
      ...
      if (generation === actionGenerationRef.current && actionIdentityKey === identityKeyRef.current) setMessage(cause instanceof Error ? cause.message : String(cause));
      return false;
    } finally {
  ...
  if (!projectId) return <section className="settings-empty-state"><Sparkles size={24} /><h3>选择项目后管理 Skills</h3><p>Skills 只影响 Coding Agent 的可用能力，不会自动执行项目操作。</p></section>;

  return (
    <section className="skills-settings-view" aria-label="Skills 设置">
      <header className="skills-page-toolbar">
        <label className="skills-search"><Search size={16} aria-hidden="true" /><span className="sr-only">搜索 Skills</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索名称、说明或来源" aria-label="搜索 Skills" />{query ? <button type="button" aria-label="清除搜索" onClick={() => setQuery("")}><X size={14} /></button> : null}</label>
        ...
        <button className="icon-button" aria-label="Skill 来源设置" title="Skill 来源设置" onClick={() => setSourceManagerOpen(true)}><Settings2 size={16} /></button>
      </header>

      {catalogErrors.length > 0 ? <div className="skills-catalog-warning" role="status"><CircleAlert size={16} /><span>有 {catalogErrors.length} 个 Skill 无法读取。</span><button type="button" onClick={() => setCatalogDiagnosticsOpen(true)}>查看诊断</button></div> : null}
      {message ? <p className="diagnostic-errors" role="alert">{safeDiagnosticMessage(message)}</p> : null}

      <div className="skills-settings-list" role="list" aria-label="Skill 列表">
        {groupedSkills.length === 0 ? <div className="skills-empty-results"><Sparkles size={22} /><strong>{query ? "没有匹配的 Skill" : "还没有扫描到 Skill"}</strong>{query ? <button className="outline-button" onClick={() => setQuery("")}>清除搜索</button> : null}</div> : groupedSkills.map((group) => <section className="skills-group" key={group.id} aria-labelledby={`skill-group-${group.id}`}><header><h3 id={`skill-group-${group.id}`}>{skillGroupLabel(group.id)}</h3><span>{group.items.length}</span></header><div className="skills-group-grid">{group.items.map((skill) => {
...
function skillGroupLabel(group: SkillGroupId): string { return group === "enabled" ? "已启用" : group === "project" ? "项目" : group === "provider" ? "Provider" : "自定义来源"; }
...
function sourceKindLabel(kind: string): string { return kind === "system-aho" ? "AHO 内置 Skills" : kind === "provider-native" ? "Provider 本地 Skills" : kind === "project-harness" ? "项目 Skills" : kind === "custom" ? "自定义来源" : kind; }
