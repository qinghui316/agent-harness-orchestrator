...
async function assertCurrentHighImpactWorkflowTarget(memory: ResolvedMemory, changeId: string, request: WorkbenchWorkflowActionRequest): Promise<void> {
  ...
  if (request.actionType === "planning.scheduler.worker-plan.compile") {
    ...
    assertLatestWorkbenchActionTarget(latestDryRun, dryRun, "planning.scheduler.worker-plan.compile", "SchedulerDispatchDryRun");
    ...
    assertLatestWorkbenchActionTarget(latestContract, contract, "planning.scheduler.worker-plan.compile", "SchedulerContract");
  }
  ...
  if (request.actionType === "planning.scheduler.launch-preflight.check") {
    ...
    assertLatestWorkbenchActionTarget(latestClaimPlan, claimPlan, "planning.scheduler.launch-preflight.check", "SchedulerClaimReconcilePlan");
    ...
    const latestWorkerPlan = await readLatestSchedulerWorkerSessionPlan(memory, target.path);
    assertLatestWorkbenchActionTarget(latestWorkerPlan, workerPlan, "planning.scheduler.launch-preflight.check", "SchedulerWorkerSessionPlan");
    ...
    const latestDryRun = await readLatestSchedulerDispatchDryRun(memory, target.path);
    assertLatestWorkbenchActionTarget(latestDryRun, dryRun, "planning.scheduler.launch-preflight.check", "SchedulerDispatchDryRun");
    ...
    const latestContract = await readLatestSchedulerContract(memory, target.path);
    assertLatestWorkbenchActionTarget(latestContract, contract, "planning.scheduler.launch-preflight.check", "SchedulerContract");
  }
  if (request.actionType === "planning.scheduler.run.prepare") {
    ...
    assertLatestWorkbenchActionTarget(latestPreflight, preflight, "planning.scheduler.run.prepare", "SchedulerLaunchPreflight");
    ...
    const latestClaimPlan = await readLatestSchedulerClaimReconcilePlan(memory, target.path);
    assertLatestWorkbenchActionTarget(latestClaimPlan, claimPlan, "planning.scheduler.run.prepare", "SchedulerClaimReconcilePlan");
    ...
    const latestWorkerPlan = await readLatestSchedulerWorkerSessionPlan(memory, target.path);
    assertLatestWorkbenchActionTarget(latestWorkerPlan, workerPlan, "planning.scheduler.run.prepare", "SchedulerWorkerSessionPlan");
    ...
    const latestDryRun = await readLatestSchedulerDispatchDryRun(memory, target.path);
    assertLatestWorkbenchActionTarget(latestDryRun, dryRun, "planning.scheduler.run.prepare", "SchedulerDispatchDryRun");
    ...
    const latestContract = await readLatestSchedulerContract(memory, target.path);
    assertLatestWorkbenchActionTarget(latestContract, contract, "planning.scheduler.run.prepare", "SchedulerContract");
  }
  ...
  if (request.actionType === "planning.scheduler.worker.start-first" || request.actionType === "planning.scheduler.worker.start-next") {
    ...
    if (!request.reservationIntentId) throw new Error(`${request.actionType} requires reservationIntentId.`);
    if (!request.claimIntentId) throw new Error(`${request.actionType} requires claimIntentId.`);
    const run = await readSchedulerRun(memory, target.path, request.schedulerRunId);
    assertPreparedWorkbenchActionTarget(run, request.schedulerRunId, changeId, request.actionType, "SchedulerRun");
    const latestRun = await readLatestSchedulerRun(memory, target.path);
    assertLatestWorkbenchActionTarget(latestRun, run, request.actionType, "SchedulerRun");
    ...
    const selectedIntent = reservation.reservationIntents.find((intent) => intent.reservationIntentId === request.reservationIntentId);
    ...
    if (selectedIntent.claimIntentId !== request.claimIntentId) {
    ...
    if (request.actionType === "planning.scheduler.worker.start-next") {
      ...
      if (await readLatestSchedulerRunCompletionProjection(memory, target.path, run.id)) {
        throw new Error("planning.scheduler.worker.start-next is blocked after SchedulerRunCompletion exists.");
      }
      if (await readLatestSchedulerRunBlockedCloseoutProjection(memory, target.path, run.id)) {
        throw new Error("planning.scheduler.worker.start-next is blocked after SchedulerRunBlockedCloseout exists.");
      }
    }
  }
  ...
  if (request.actionType === "planning.scheduler.worker.validate-first") {
    ...
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerStartId, workerResult.schedulerWorkerStartId, "planning.scheduler.worker.validate-first", "WorkerStart");
    assertWorkbenchActionOptionalStringTarget(request.schedulerClaimReservationId, workerResult.schedulerClaimReservationId, "planning.scheduler.worker.validate-first", "SchedulerRuntimeClaimReservation");
    assertWorkbenchActionOptionalStringTarget(request.reservationIntentId, workerResult.reservationIntentId, "planning.scheduler.worker.validate-first", "reservationIntentId");
    assertWorkbenchActionOptionalStringTarget(request.claimIntentId, workerResult.claimIntentId, "planning.scheduler.worker.validate-first", "claimIntentId");
    assertWorkbenchActionOptionalStringTarget(request.taskRunId, workerResult.taskRunId, "planning.scheduler.worker.validate-first", "TaskRun");
    assertWorkbenchActionOptionalStringTarget(request.workerLeaseId, workerResult.workerLeaseId, "planning.scheduler.worker.validate-first", "WorkerLease");
    assertWorkbenchActionOptionalStringTarget(request.worktreeId, workerResult.worktreeId ?? "", "planning.scheduler.worker.validate-first", "worktree");
    assertWorkbenchActionOptionalStringTarget(request.runId, workerResult.runId ?? "", "planning.scheduler.worker.validate-first", "code run");
  ...
  if (request.actionType === "planning.scheduler.worker.audit-first") {
    ...
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerStartId, workerValidation.schedulerWorkerStartId, "planning.scheduler.worker.audit-first", "WorkerStart");
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerResultId, workerValidation.schedulerWorkerResultId, "planning.scheduler.worker.audit-first", "WorkerResult");
    assertWorkbenchActionOptionalStringTarget(request.schedulerClaimReservationId, workerValidation.schedulerClaimReservationId, "planning.scheduler.worker.audit-first", "SchedulerRuntimeClaimReservation");
    assertWorkbenchActionOptionalStringTarget(request.reservationIntentId, workerValidation.reservationIntentId, "planning.scheduler.worker.audit-first", "reservationIntentId");
    assertWorkbenchActionOptionalStringTarget(request.claimIntentId, workerValidation.claimIntentId, "planning.scheduler.worker.audit-first", "claimIntentId");
    assertWorkbenchActionOptionalStringTarget(request.taskRunId, workerValidation.taskRunId, "planning.scheduler.worker.audit-first", "TaskRun");
    assertWorkbenchActionOptionalStringTarget(request.workerLeaseId, workerValidation.workerLeaseId, "planning.scheduler.worker.audit-first", "WorkerLease");
    assertWorkbenchActionOptionalStringTarget(request.worktreeId, workerValidation.worktreeId, "planning.scheduler.worker.audit-first", "worktree");
    assertWorkbenchActionOptionalStringTarget(request.runId, workerValidation.codeRunId, "planning.scheduler.worker.audit-first", "code run");
    assertWorkbenchActionOptionalStringTarget(request.validationRunId, workerValidation.validationRunId, "planning.scheduler.worker.audit-first", "validation run");
  ...
  if (request.actionType === "planning.scheduler.worker.rework-validate-first") {
    ...
    assertWorkbenchActionOptionalStringTarget(request.schedulerClaimReservationId, reworkResult.schedulerClaimReservationId, "planning.scheduler.worker.rework-validate-first", "SchedulerRuntimeClaimReservation");
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerReworkPlanId, reworkResult.schedulerWorkerReworkPlanId, "planning.scheduler.worker.rework-validate-first", "WorkerReworkPlan");
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerReworkStartId, reworkResult.schedulerWorkerReworkStartId, "planning.scheduler.worker.rework-validate-first", "WorkerReworkStart");
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerValidationId, reworkResult.schedulerWorkerValidationId, "planning.scheduler.worker.rework-validate-first", "WorkerValidation");
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerAuditId, reworkResult.schedulerWorkerAuditId ?? "", "planning.scheduler.worker.rework-validate-first", "WorkerAudit");
    assertWorkbenchActionOptionalStringTarget(request.reservationIntentId, reworkResult.reservationIntentId, "planning.scheduler.worker.rework-validate-first", "reservationIntentId");
    assertWorkbenchActionOptionalStringTarget(request.claimIntentId, reworkResult.claimIntentId, "planning.scheduler.worker.rework-validate-first", "claimIntentId");
    assertWorkbenchActionOptionalStringTarget(request.taskRunId, reworkResult.reworkTaskRunId, "planning.scheduler.worker.rework-validate-first", "rework TaskRun");
    assertWorkbenchActionOptionalStringTarget(request.workerLeaseId, reworkResult.reworkWorkerLeaseId, "planning.scheduler.worker.rework-validate-first", "rework WorkerLease");
    assertWorkbenchActionOptionalStringTarget(request.worktreeId, reworkResult.worktreeId, "planning.scheduler.worker.rework-validate-first", "worktree");
    assertWorkbenchActionOptionalStringTarget(request.runId, reworkResult.reworkRunId ?? "", "planning.scheduler.worker.rework-validate-first", "rework code run");
  ...
  if (request.actionType === "planning.scheduler.worker.rework-audit-first") {
    ...
    assertWorkbenchActionOptionalStringTarget(request.schedulerClaimReservationId, reworkValidation.schedulerClaimReservationId, "planning.scheduler.worker.rework-audit-first", "SchedulerRuntimeClaimReservation");
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerReworkPlanId, reworkValidation.schedulerWorkerReworkPlanId, "planning.scheduler.worker.rework-audit-first", "WorkerReworkPlan");
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerReworkStartId, reworkValidation.schedulerWorkerReworkStartId, "planning.scheduler.worker.rework-audit-first", "WorkerReworkStart");
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerReworkResultId, reworkValidation.schedulerWorkerReworkResultId, "planning.scheduler.worker.rework-audit-first", "WorkerReworkResult");
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerValidationId, reworkValidation.schedulerWorkerValidationId, "planning.scheduler.worker.rework-audit-first", "WorkerValidation");
    assertWorkbenchActionOptionalStringTarget(request.schedulerWorkerAuditId, reworkValidation.schedulerWorkerAuditId ?? "", "planning.scheduler.worker.rework-audit-first", "WorkerAudit");
    assertWorkbenchActionOptionalStringTarget(request.reservationIntentId, reworkValidation.reservationIntentId, "planning.scheduler.worker.rework-audit-first", "reservationIntentId");
    assertWorkbenchActionOptionalStringTarget(request.claimIntentId, reworkValidation.claimIntentId, "planning.scheduler.worker.rework-audit-first", "claimIntentId");
    assertWorkbenchActionOptionalStringTarget(request.taskRunId, reworkValidation.reworkTaskRunId, "planning.scheduler.worker.rework-audit-first", "rework TaskRun");
    assertWorkbenchActionOptionalStringTarget(request.workerLeaseId, reworkValidation.reworkWorkerLeaseId, "planning.scheduler.worker.rework-audit-first", "rework WorkerLease");
    assertWorkbenchActionOptionalStringTarget(request.worktreeId, reworkValidation.worktreeId, "planning.scheduler.worker.rework-audit-first", "worktree");
    assertWorkbenchActionOptionalStringTarget(request.runId, reworkValidation.reworkRunId, "planning.scheduler.worker.rework-audit-first", "rework code run");
    assertWorkbenchActionOptionalStringTarget(request.reworkValidationRunId, reworkValidation.validationRunId, "planning.scheduler.worker.rework-audit-first", "rework validation run");
  ...
  if (request.actionType === "planning.scheduler.integration-candidate.compile") {
    ...
    await assertSchedulerCurrentTransitionForAction(memory, target.path, run.id, reservation, "planning.scheduler.integration-candidate.compile");
  ...
  if (request.actionType === "planning.scheduler.integration-check.run") {
    ...
    await assertSchedulerCurrentTransitionForAction(memory, target.path, run.id, reservation, "planning.scheduler.integration-check.run");
  ...
  if (request.actionType === "planning.scheduler.integration-outcome.reconcile") {
    ...
    assertWorkbenchActionOptionalStringTarget(request.applyCheckId, latestHandoff.integrationCheckId, "planning.scheduler.integration-outcome.reconcile", "applyCheckId");
  ...
  if (request.actionType === "planning.scheduler.run.complete") {
    ...
    await assertSchedulerCurrentTransitionForAction(memory, target.path, run.id, reservation, "planning.scheduler.run.complete");
    const latestOutcome = await readLatestSchedulerIntegrationOutcomeProjection(memory, target.path, run.id);
    ...
    assertWorkbenchActionOptionalStringTarget(request.schedulerReconcileSnapshotId, outcome.schedulerReconcileSnapshotId, "planning.scheduler.run.complete", "schedulerReconcileSnapshotId");
    assertWorkbenchActionOptionalStringTarget(request.schedulerClaimReservationId, outcome.schedulerClaimReservationId, "planning.scheduler.run.complete", "schedulerClaimReservationId");
    assertWorkbenchActionOptionalStringTarget(request.schedulerIntegrationCandidateId, outcome.schedulerIntegrationCandidateId, "planning.scheduler.run.complete", "SchedulerIntegrationCandidate");
    assertWorkbenchActionOptionalStringTarget(request.schedulerIntegrationCheckHandoffId, outcome.schedulerIntegrationCheckHandoffId, "planning.scheduler.run.complete", "SchedulerIntegrationCheckHandoff");
    ...
    assertWorkbenchActionOptionalStringTarget(request.applyCheckId, outcome.integrationCheckId, "planning.scheduler.run.complete", "applyCheckId");
  ...
  if (request.actionType === "planning.scheduler.run.close-blocked") {
    ...
    await assertSchedulerCurrentTransitionForAction(memory, target.path, run.id, reservation, "planning.scheduler.run.close-blocked");
...
async function assertSchedulerCurrentTransitionForAction(
  ...
  reservation: { reservationIntents: Array<{ reservationIntentId: string; claimIntentId: string; status: string; waveIndex: number; sourceScopes?: string[] }> },
  actionType: string,
): Promise<void> {
  const paths = schedulerWorkerPathsToLikes(await readSchedulerWorkerPathReadModelsForReservation(memory, changePath, schedulerRunId, reservation));
  const integrationCandidate = await readLatestSchedulerIntegrationCandidateProjection(memory, changePath, schedulerRunId);
  const latestGraph = await readLatestWorkflowGraphPlan(memory, changePath).catch(() => null);
  const transition = resolveSchedulerCurrentTransition({
    graph: latestGraph?.graphMode === "ready-set-v1" ? latestGraph : null,
    reservation,
    workerPaths: paths,
    integrationCandidate,
    integrationCandidateNeedsRefresh: integrationCandidate ? schedulerIntegrationCandidateNeedsRefresh(integrationCandidate, paths) : true,
    integrationCheckHandoffExists: Boolean(await readLatestSchedulerIntegrationCheckHandoffProjection(memory, changePath, schedulerRunId)),
    integrationOutcomeExists: Boolean(await readLatestSchedulerIntegrationOutcomeProjection(memory, changePath, schedulerRunId)),
    runCompletionExists: Boolean(await readLatestSchedulerRunCompletionProjection(memory, changePath, schedulerRunId)),
    runBlockedCloseoutExists: Boolean(await readLatestSchedulerRunBlockedCloseoutProjection(memory, changePath, schedulerRunId)),
  });
  if (transition.actionType !== actionType) {
    throw new Error(`${actionType} is not the current Scheduler transition.`);
  }
}
