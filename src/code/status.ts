...
export async function getCodeStatus(project: ManagedProject): Promise<CodeStatusResult> {
  ...
  const runs = (await listRuns(project)).filter((run) => run.runtime === "provider-code" && (!changeId || run.changeId === changeId));
...
export async function listCodeRuns(project: ManagedProject): Promise<RunMetadata[]> {
  return (await listRuns(project)).filter((run) => run.runtime === "provider-code");
}
