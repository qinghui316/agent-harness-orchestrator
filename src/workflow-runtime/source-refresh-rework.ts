...
export async function runSourceRefreshReworkWorkflow(input: SourceRefreshReworkWorkflowInput): Promise<SourceRefreshReworkWorkflowResult> {
  ...
  if (created) {
    await appendMainAgentLoopEvent(memory, loopRun, {
      type: "loop.started",
      entrypoint: "source-refresh-rework",
      summary: "Source-refresh rework runtime loop started.",
    });
  }
  ...
  if (validation.status === "failed" || !validation.validation) {
    const decision = terminalNeedsUserInput("validation", validation.error ?? "Validation failed during source-refresh rework.");
    await recordDecision(memory, loopRun, stepIndex + 1, orchestration, decision, {
  ...
  if (audit.status === "failed" || !audit.audit) {
    const decision = terminalNeedsUserInput("audit", audit.error ?? "Audit failed during source-refresh rework.");
    await recordDecision(memory, loopRun, stepIndex + 1, orchestration, decision, {
  ...
  const completedDecision: MainAgentOrchestrationDecision = {
    kind: "completed",
    reason: "Source-refresh rework completed validation and audit.",
...
async function appendLeafStarted(
  ...
  await appendMainAgentLoopEvent(memory, loopRun, {
    ...
    summary: `Source-refresh rework delegated ${decision.roleId}.`,
...
function observationSummary(orchestration: MainAgentOrchestrationState): MainAgentNextStepObservationSummary {
  ...
  return {
    summary: latest
      ? `Source-refresh rework has ${completedSteps} completed step(s) and ${failedSteps} failed step(s); latest ${latest.roleId} ${latest.status}.`
      : "Source-refresh rework has no role evidence yet.",
...
function terminalDecisionForCodeFailure(stoppedAt: "boundary" | "code" | "validation" | "audit", reason: string | undefined): MainAgentOrchestrationDecision {
  if (stoppedAt === "boundary" || stoppedAt === "code") {
    return {
      kind: "failed",
      stoppedAt,
      reason: reason ?? "Source-refresh rework did not produce a completed worktree proposal.",
      nextRecommendation: "Surface the failure evidence to the user.",
    };
  }
  return terminalNeedsUserInput(stoppedAt, reason ?? "Source-refresh rework stopped before completion.");
}
